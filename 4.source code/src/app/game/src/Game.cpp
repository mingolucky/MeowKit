...
void Game::begin(DEVICES *device)
{
    ...
    _framebuffer = new LGFX_Sprite(&_device->Lcd);
...
void Game::_play()
{
    ...
    // if (button.A.pressed() && !_dino->jumping()) {
    //     _dino->jump();
    // }
    if (_device->ctp.isTouched() && !_dino->jumping())
    {
        _dino->jump();
    }
...
void Game::_spawnCubes()
{
    ...
    if ((c->solid && c->x + _speed * _MIN_GAP > TFT_WIDTH) || _dino->y() > _BASELINE)
        return;
...
void Game::_spawnCube(uint8_t const x, uint8_t const floor, bool const cracked)
{
    ...
    if (_cubes < _MAX_CUBE_NUMBER)
    {
        ...
        _last_cube_index++;
        if (_last_cube_index == _MAX_CUBE_NUMBER)
            _last_cube_index = 0;
...
void Game::_drawSplash()
{
    ...
    _drawString("PRESS SCREEN", TFT_WIDTH >> 1, y + SPLASH_HEIGHT + 8, 0xffff, TextAlign::CENTER);
...
void Game::_drawGameOver()
{
    ...
    char highscore[6];
    snprintf(highscore, 6, "%u", _backup_data.highscore);
