...
namespace MOONCAKE::APPS
{
    ...
    void VUMeter::setPointerAngle_(float deg) {
        ...
        if (std::abs(deg - lastAngle) < 1.0f) {  // Increase the threshold to 1 degree
    ...
    void VUMeter::onOpen()
    {
        ...
        for (int cycle = 0; cycle < 1; cycle++) {  // Reduce the number of cycles
            for (int deg = -60; deg <= 60; deg += 5) {  // Increase step size to reduce ghosting
    ...
    void VUMeter::updateVu_() {
        ...
        if (err != ESP_OK || bytesRead == 0) {
            ...
            if (currentTime - lastRefreshTime > 50) {  // 20FPS refresh
                lv_refr_now(lv_disp_get_default());
        ...
        if (amplitude > 0) {
            ...
            if (currentTime - lastRefreshTime > 33) {  // About 30FPS refresh rate
                lv_refr_now(lv_disp_get_default());
                lastRefreshTime = currentTime;
    ...
    void VUMeter::onRunning()
    {
        ...
        if (_device && millis() - testModeStart > 10000 && !testMode) {  // Enter the test in 10 seconds
            testMode = true;
        }
        ...
        if (testMode) {
            ...
            uint32_t currentTime = millis();
            if (currentTime - lastTestUpdate > 100) {  // Reduce to 10Hz update, more natural
                testAngle += testDirection * 1.5f;  // Reduce step size
                
                if (testAngle >= 60.0f) {     // Changed to +60° range
                    testAngle = 60.0f;
                    testDirection = -1.0f;
                } else if (testAngle <= -60.0f) {  //Changed to -60° range
                    testAngle = -60.0f;
                    testDirection = 1.0f;
                }
                ...
                if (currentTime % 200 == 0) {  // Refresh every 200ms
                    lv_refr_now(lv_disp_get_default());
                }
