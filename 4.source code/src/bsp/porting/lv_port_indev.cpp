...
static CTP* _ctp;
static lv_group_t* s_keypad_group = nullptr;
/**********************
 *      MACROS
 **********************/
...
void lv_port_indev_init(CTP* ctp)
{
    ...
    // 注册 LVGL 触摸输入设备（独立的 driver 结构体，避免被后续键盘覆盖）
    ...
    // Create a default group, so UI can add focusable objects easily
    s_keypad_group = lv_group_create();
    lv_group_set_default(s_keypad_group);
    lv_indev_set_group(indev_keypad, s_keypad_group);
...
static void keypad_init(void)
{
    pinMode(HAL_A, INPUT_PULLUP);
    pinMode(HAL_B, INPUT_PULLUP);
}
...
static uint32_t keypad_get_key(void)
{
    ...
    if(digitalRead(HAL_A) == LOW) {
        return LV_KEY_HOME; // Map A -> HOME
    }
    if(digitalRead(HAL_B) == LOW) {
        return LV_KEY_ESC;  // Map B -> ESC
    }
    return 0; // no key
}
